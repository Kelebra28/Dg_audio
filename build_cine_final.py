...
def get_section_by_class(soup, class_name):
    for s in soup.find_all('section'):
        if class_name in s.get('class', []):
            return s
    return None
...
# Section 2: Apertura emocional (two-col + benefits-corp-grid)
...
# Map to the section with id="incluye" or class="include-card"
ex_s4 = None
for s in ex_sections:
    if "Qué incluye la solución" in s.text:
        ex_s4 = s
        break
...
# Remove the extra "Qué hacemos" section from the template if it exists to avoid duplication
# audio-distribuido.html has an extra section-white with "Qué hacemos" between problema and incluye
extra_section = soup.find('section', class_='section-white') # The first one after problema
if extra_section and not extra_section.find(class_='area-grid'):
    # If it's the one with benefit-v2-card and "Qué hacemos" eyebrow, delete it
...
# The areas section in template has area-grid inside a section-white
s5 = None
for s in soup.find_all('section'):
...
if s6 and ex_s6:
    ...
    if s6.find('img') and ex_s6.find('img'): s6.find('img')['src'] = ".." + ex_s6.find('img')['src']
...
print("Script final ejecutado exitosamente.")
