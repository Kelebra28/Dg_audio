...
# 0: hero
# 1: bg-white (Apertura emocional)
# 2: bg-soft (Problema)
# 3: bg-white (Qué hacemos) -- maybe? Let's check.

# Helper to get main paragraphs (excluding cards)
def get_main_paragraphs(section):
    # Find all p tags that are NOT inside a card element
    ps = []
    for p in section.find_all('p'):
        ...
        if not any('card' in c for c in parent_classes) and not any('step' in c for c in parent_classes):
            ps.append(p)
    return ps

# HERO (soup s[0], ex s[0])
...
# Section 2: Apertura emocional (soup s[1], ex s[1])
...
if s2 and ex_s2:
    ...
    # Filter soup's ps to only those NOT in cards
    soup_main_ps = []
    for p in ps:
...
# Section 3: Antitesis (soup s[2], ex s[2])
...
# The "qué incluye" is soup[3]. In the example, it might be section 4.
# Let's map by searching for the "feature-card" or "include-card"
ex_s4 = None
for s in ex_sections:
    if s.find(class_='feature-card'):
        ex_s4 = s
        break
...
if s4 and ex_s4:
    ...
    # The paragraph in s4 might be under container.
    soup_p = None
...
# Let's find the section that has "No vendemos cajas" and "Una decisión profesional"
ex_s8_dark = None
ex_s8_gold = None
for s in ex_sections:
    h2 = s.find('h2')
