...
if s4 and ex_s4:
    ...
    # If there are more include-cards in soup than example, decompose extras
    # If there are more in example than soup, we add them
    if len(ex_incl) > len(incl):
        grid = incl[0].parent
    ...
    for i, ic in enumerate(incl):
        if i < len(ex_incl):
            ex_h3 = ex_incl[i].find('strong').text
...
if s7 and ex_s7:
    ...
    # if more steps in example, add them
    if len(ex_steps) > len(b_items):
...
print("Modificaciones listas con EXACTAMENTE los textos e imágenes del archivo de ejemplo V5.")
