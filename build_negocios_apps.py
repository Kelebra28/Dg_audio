...
def process_app(example_path, output_filename):
    ...
    if s5 and ex_s5:
        ...
        if area_grid and ex_areas:
            ...
            for ex_a in ex_areas:
                ...
                img_tag = soup.new_tag('img', src=fix_img_src(ex_a.find('img')['src']), alt=ex_a.find('img').get('alt', ''))
                span_tag = soup.new_tag('span')
                if ex_a.find('span'): span_tag.string = ex_a.find('span').text
    ...
    ex_s6 = None
    for s in ex_sections:
        if s.find(class_='event-img'):
            ex_s6 = s
            break

    s6 = None
    for s in soup.find_all('section'):
        if s.find(class_='event-banner'):
            s6 = s
            break
    ...
    for s in ex_sections:
        h2 = s.find('h2')
        if h2 and "No vendemos cajas" in h2.text:
            ex_s8_dark = s
        if h2 and "decisión profesional" in h2.text:
            ex_s8_gold = s
...
mappings = [
    ("example/WEB DG Audiosound/01 Home/aplicaciones/negocios-y-experiencias-comerciales/audio-video-restaurantes-dg-audiosound.html", "restaurantes.html"),
    ("example/WEB DG Audiosound/01 Home/aplicaciones/negocios-y-experiencias-comerciales/audio-video-iluminacion-bares-antros-dg-audiosound.html", "bares-antros.html"),
    ("example/WEB DG Audiosound/01 Home/aplicaciones/negocios-y-experiencias-comerciales/audio-video-barberias-dg-audiosound.html", "barberias.html"),
    ("example/WEB DG Audiosound/01 Home/aplicaciones/negocios-y-experiencias-comerciales/audio-video-cafeterias-dg-audiosound.html", "cafeterias.html"),
    ("example/WEB DG Audiosound/01 Home/aplicaciones/negocios-y-experiencias-comerciales/audio-video-retail-dg-audiosound.html", "retail.html"),
    ("example/WEB DG Audiosound/01 Home/aplicaciones/negocios-y-experiencias-comerciales/audio-video-gimnasios-dg-audiosound.html", "gimnasios.html")
]

for src, out in mappings:
    process_app(src, out)

print("Todas las aplicaciones fueron creadas con exito.")
